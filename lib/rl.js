...
DQNAgent.prototype = {
  reset: function() {
    ...
    this.latestAmat = zeros(this.na); // same
  },
  toJSON: function() {
    // save function
    var j = {};
  ...
  act: function(slist) {
    ...
    var poss = this.env.allowedActions 
      ? this.env.allowedActions(slist) : all;

    // epsilon greedy policy
    if(Math.random() < this.epsilon) {
      // var a = randi(0, this.na);
      var a = poss[randi(0, poss.length)];
    } else {
      ...
      // visualization
      this.latestAmat = amat.w;
      var validW = amat.w.filter(function(el, i) { return poss.includes(i); });
      var a = poss[R.maxi(validW)];
    }
